...
const TOP_PRICE = 300;    // rows A–F
const BOTTOM_PRICE = 200; // rows G–P
...
const ROW_SHAPE = {
    ...
    N: { block: "bottom", left: 5, middle: 13, right: 5 },
    O: { block: "bottom", left: 3, middle: 13, right: 3 },

    P: { block: "bottom", left: 3, middle: 9, right: 3, hasAisles: false },
};
...
function createSegment(side, realCount, maxCount, rowLabel, seatRow, seatNumberRef) {
...
function createRow(rowLabel, shape, container) {
    ...
    createSegment("left", shape.left, SEGMENT_MAX.left, rowLabel, seatRow, seatNumberRef);
    if (shape.hasAisles !== false) { const a = document.createElement("div"); a.className = "aisle-vert"; seatRow.appendChild(a); }
    createSegment("middle", shape.middle, SEGMENT_MAX.middle, rowLabel, seatRow, seatNumberRef);
    if (shape.hasAisles !== false) { const a = document.createElement("div"); a.className = "aisle-vert"; seatRow.appendChild(a); }
    createSegment("right", shape.right, SEGMENT_MAX.right, rowLabel, seatRow, seatNumberRef);
...
function markSeatsAsBooked(seatsArray, { dropFromSelection = true } = {}) {
    ...
    document.querySelectorAll(".seat").forEach(el => {
        ...
        if (!id) return;
        if (preBooked.has(id)) { el.classList.add("booked"); el.classList.remove("selected"); }
    });
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("click", (e) => { if (e.target === document.querySelector(".modal-backdrop")) modal.style.display = "none"; });
